...
unique_ptr<fractal_t> g_triangle;
...
void display_func()
{
   ...
   g_triangle->draw_frame(chrono::duration<float>(chrono::system_clock::now() - start).count());
...
void special_func(int key, int x, int y) {
    ...
    switch (key) {
    case GLUT_KEY_RIGHT:
        g_triangle->move(vec2(-1, 0));
        break;
    case GLUT_KEY_LEFT:
        g_triangle->move(vec2(1, 0));
        break;
    case GLUT_KEY_UP:
        g_triangle->move(vec2(0, -1));
        break;
    case GLUT_KEY_DOWN:
        g_triangle->move(vec2(0, 1));
        break;
...
int main( int argc, char ** argv )
{
   ...
   try
   {
      g_triangle.reset(new fractal_t());
